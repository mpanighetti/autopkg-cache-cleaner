...
/bin/echo "Cutoff for aging files: $oldCutoff days"
...
if [ ! -d "$autopkgCacheDir" ]; then
  /bin/echo "❌ ERROR: AutoPkg cache directory not found at $autopkgCacheDir, unable to proceed."
  exit 1
else
...
if [ -z "$deleteTheseFiles" ]; then
  ...
  /bin/echo ""
  /bin/echo "$deleteTheseFiles" | while read -r deleteMe; do
...
/bin/echo ""
/bin/echo "Space saved: $oldCacheSizeLCD"
/bin/echo ""
